...
#define get_value(kv) ((int64_t*)(kv+FINGERPRINT_LENGTH))

#define INDEX_VALUE_LENGTH (1)
...
static kvpair new_kvpair_full(char* key){
    ...
    memcpy(get_key(kvp), key, FINGERPRINT_LENGTH);
    int64_t* values = get_value(kvp);
    int i;
    for(i = 0; i<INDEX_VALUE_LENGTH; i++){
...
static void kv_update(kvpair kv, int64_t id){
	...
	memmove(&value[1], value,
			(INDEX_VALUE_LENGTH - 1) * sizeof(int64_t));
	value[0] = id;
}
...
{
    return !memcmp(a, b, FINGERPRINT_LENGTH);
}

void init_kvstore_htable(){
    kvpair_size = FINGERPRINT_LENGTH + INDEX_VALUE_LENGTH * 8;

	htable = g_hash_table_new_full(g_int_hash, (void *)g_feature_equal,
		(void *)free_kvpair, NULL);

	char* indexpath = "/home/kid/CODE/EncryptCloudStorge/kvstore/htable";
...
void close_kvstore_htable() {
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		...
		/* Write a feature. */
		kvpair kv = value;
		if(fwrite(get_key(kv), FINGERPRINT_LENGTH, 1, fp) != 1){
			perror("Fail to write a key!");
			exit(1);
		}
		int index_value_length = INDEX_VALUE_LENGTH;
		/* Write the number of segments/containers */
		if(fwrite(&index_value_length, sizeof(int), 1, fp) != 1){
...
void kvstore_htable_delete(char* key, int64_t id){
	...
	for(i=0; i<INDEX_VALUE_LENGTH; i++){
		if(value[i] == id){
			...
			assert((i == INDEX_VALUE_LENGTH - 1)
					|| value[i+1] == TEMPORARY_ID);
			if(i < INDEX_VALUE_LENGTH - 1 && value[i+1] != TEMPORARY_ID){
				/* If the next ID is not TEMPORARY_ID */
				memmove(&value[i], &value[i+1], (INDEX_VALUE_LENGTH - i - 1) * sizeof(int64_t));
			}
			break;
		}
	}
